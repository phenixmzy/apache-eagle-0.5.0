...
class KafkaLog4jAppender extends AppenderSkeleton with Logging {
  ...
  override def activateOptions() {
    ...
    if(serializerClass == null) {
      ...
      LogLog.debug("Using default encoder - org.apache.kafka.common.serialization.StringSerializer")
    }
    ...
    if(keyClass != null){
      keyer = CoreUtils.createObject[Keyer](keyClass, extraProps)
      LogLog.debug("Instantiated Key class " +  keyClass)
    }

    if(topicClass != null) {
      try {
        topicPicker = CoreUtils.createObject[TopicPicker](topicClass, props)
