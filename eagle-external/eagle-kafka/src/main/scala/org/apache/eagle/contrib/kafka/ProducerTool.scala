...
object ProducerTool {
  def main(args: Array[String]) {
    ...
    try {
      ...
      /**
        * modify by mzy.
        * Old interface in 0.8.1.
        * val producer = new Producer[AnyRef, AnyRef](new ProducerConfig(props))
        * */
      val producer = new KafkaProducer[scala.Array[scala.Byte], scala.Array[scala.Byte]](props)
      Runtime.getRuntime.addShutdownHook(new Thread() {
        override def run() {
          producer.close()
        }
      ...
      /**
        ...
        * */
      var message: ProducerRecord[scala.Array[scala.Byte], scala.Array[scala.Byte]] = null
      do {
        message = reader.readMessage()
        if (message != null)
          producer.send(message)
