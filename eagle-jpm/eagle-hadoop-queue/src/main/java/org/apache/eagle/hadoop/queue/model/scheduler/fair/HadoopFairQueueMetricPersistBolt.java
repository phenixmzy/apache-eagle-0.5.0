...
public class HadoopFairQueueMetricPersistBolt extends BaseRichBolt {

    private static final Logger LOG = LoggerFactory.getLogger(HadoopFairQueueMetricPersistBolt.class);

    private HadoopQueueRunningAppConfig config;
    private IEagleServiceClient client;
    private OutputCollector collector;

    public HadoopFairQueueMetricPersistBolt(HadoopQueueRunningAppConfig config) {
        this.config = config;
    }
    ...
    public void execute(Tuple input) {
        ...
        if (dataType.equals(DataType.METRIC)) {
            ...
            for (TaggedLogAPIEntity entity : entities) {
                if (entity instanceof FairRunningQueueAPIEntity) {
                    FairRunningQueueAPIEntity queue = (FairRunningQueueAPIEntity) entity;
                    if (queue.getUsedMemory() != 0) {
                        String queueName = queue.getTags().get(HadoopClusterConstants.TAG_QUEUE);
                        collector.emit(new Values(queueName, FairQueueStreamInfo.convertEntityToStream(queue)));
                    }
                }
            }
            writeEntities(entities, dataType, dataSource);
