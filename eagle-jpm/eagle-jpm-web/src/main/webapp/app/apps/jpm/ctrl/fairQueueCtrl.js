...
(function () {
	...
	register(function (jpmApp) {
		...
		jpmApp.controller("fairQueueCtrl", function ($q, $wrapState, $element, $scope, $timeout, PageConfig, Time, Entity, JPM) {
			var TEXT_MAX_CAPACITY = 'Max Capacity';
			var DISPLAY_MARK_NAME = ['Guaranteed Fair', TEXT_MAX_CAPACITY];
			...
			(function () {
				...
				JPM
					.groups('RunningFairQueueService', { site: $scope.site, queue: $scope.currentQueue }, ['queue', 'parentQueue'], 'count', 60, startTime, endTime)
					._promise
					.then(function (list) {
			...
			$scope.refresh = function () {
				...
				promiseList.push(JPM.aggMetricsToEntities(
					JPM.groups('RunningFairQueueService', condition, ['queue', 'parentQueue'], 'avg(absoluteUsedCapacity)', intervalMin, startTime, endTime)
				...
				if ($scope.currentQueue) {
					...
					promiseList.push(JPM.aggMetricsToEntities(
						JPM.groups('RunningFairQueueService',
