...
public class StormExecutionRuntime implements ExecutionRuntime<StormEnvironment,StormTopology> {
    ...
    private org.apache.storm.Config getStormConfig(com.typesafe.config.Config config) {
        ...
        conf.put(Config.TOPOLOGY_WORKER_SHARED_THREAD_POOL_SIZE, Int.box(8));
    ...
    public ApplicationEntity.Status status(Application<StormEnvironment, StormTopology> executor, com.typesafe.config.Config config) {
        ...
        try {
            if (Objects.equals(config.getString("mode"), ApplicationEntity.Mode.CLUSTER.name())) {
                org.apache.storm.Config stormConfig = getStormConfig(config);
                Nimbus.Client stormClient = NimbusClient.getConfiguredClient(stormConfig).getClient();
                LOG.info("--status method:Nimbus Client Get Leader {}--",stormClient.getLeader());
